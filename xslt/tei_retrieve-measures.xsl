    ...
    <xsl:variable name="v_data-source">
        <xsl:choose>
            ...
            <xsl:when test="$p_test = true()">
                <xsl:copy-of
                    select="collection('/Volumes/Dessau HD/BachCloud/BTSync/FormerDropbox/FoodRiots/food-riots_tools/examples/tss?select=*.TSS.xml')[not(descendant::tss:publicationType[@name = 'Archival File' or @name = 'Archival Material'])][descendant::tei:measureGrp]"
                />
            </xsl:when>
    ...
    <xsl:template match="/">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$p_commodity != '' and $p_unit != ''">
                <xsl:result-document href="_output/measureGrp_{$p_commodity}-{$p_unit}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.xml" format="xml">
                    <tei:div>
                        <xsl:for-each
                            select="$v_input/descendant::tei:measureGrp[tei:measure[@commodity = $p_commodity][@unit = $p_unit]]">
                ...
                <xsl:result-document href="_output/prices_{$p_commodity}-{$p_unit}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.csv" format="text">
                    <xsl:value-of select="$v_csv-head"/>
                    <xsl:apply-templates
                        select="$v_data-source-regularized/descendant::tei:measureGrp[tei:measure[@commodity = $p_commodity][@unit = $p_unit]]"
                        mode="m_tei-to-csv">
                        <xsl:sort select="@when"/>
                        <xsl:sort select="@location"/>
                    </xsl:apply-templates>
            ...
            <xsl:when test="$p_commodity != ''">
                <xsl:result-document href="_output/measureGrp_{$p_commodity}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.xml" format="xml">
                    <tei:div>
                        <xsl:for-each
                            select="$v_input/descendant::tei:measureGrp[tei:measure[@commodity = $p_commodity]]">
                            <xsl:sort select="@when"/>
                            <xsl:sort select="@location"/>
                            <xsl:copy-of select="."/>
                        </xsl:for-each>
                ...
                <xsl:result-document href="_output/prices_{$p_commodity}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.csv" format="text">
                    <xsl:value-of select="$v_csv-head"/>
                    <xsl:apply-templates
                        select="$v_data-source-regularized/descendant::tei:measureGrp[tei:measure[@commodity = $p_commodity]]"
                        mode="m_tei-to-csv">
                        <xsl:sort select="@when"/>
                        <xsl:sort select="@location"/>
                    </xsl:apply-templates>
            ...
            <xsl:when test="$p_unit != ''">
                <xsl:result-document href="_output/measureGrp_{$p_unit}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.xml" format="xml">
                    <tei:div>
                        <xsl:for-each
                            select="$v_input/descendant::tei:measureGrp[tei:measure[@unit = $p_unit]]">
                            <xsl:sort select="@when"/>
                            <xsl:sort select="@location"/>
                            <xsl:copy-of select="."/>
                        </xsl:for-each>
                ...
                <xsl:result-document href="_output/prices_{$p_unit}-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.csv" format="text">
                    <xsl:value-of select="$v_csv-head"/>
                    <xsl:apply-templates
                        select="$v_data-source-regularized/descendant::tei:measureGrp[tei:measure[@unit = $p_unit]]"
                        mode="m_tei-to-csv">
                        <xsl:sort select="@when"/>
                        <xsl:sort select="@location"/>
                    </xsl:apply-templates>
            ...
            <xsl:otherwise>
                <xsl:result-document href="_output/measureGrp-{
                    format-date(current-date(),'[Y0001]-[M01]-[D01]')}.xml" format="xml">
                    <tei:div>
                        <xsl:for-each select="$v_input/descendant::tei:measureGrp[tei:measure]">
                            <xsl:sort select="@when"/>
                            <xsl:sort select="@location"/>
                            <xsl:copy-of select="."/>
                        </xsl:for-each>
